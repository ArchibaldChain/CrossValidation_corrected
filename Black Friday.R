...

{
  ...
  ## sampling from datasets ## comments by Archibald
...
for(j in 1:model_no){
  ...
  Var_model_j<-Z%*%diag(unclass(VarCorr(Prefit))$'Data_Tr$ID'[],ncol(Z))%*%t(Z)+diag(attr(VarCorr(Prefit),"sc")^2,n) # don't understand now
 ## how to compute variance of Var_model  
